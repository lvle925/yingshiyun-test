...
# --- 紫微报告请求模型 ---
class ZiweiReportRequest(BaseModel):
    """紫微报告请求模型"""
    session_id: Optional[str] = Field(None, description="会话ID")
...
# --- 紫微年运请求模型 ---
class ZiweiYearRequest(BaseModel):
    """紫微年运请求模型"""
    session_id: Optional[str] = Field(None, description="会话ID")
    gender: str = Field(..., description="性别：'男' 或 '女'")
    birthday: str = Field(..., description="出生日期时间，格式：'YYYY-MM-DD HH:MM:SS'，例如：'1995-05-06 14:30:00'")
...
# --- 别名定义，确保与 Router 中的引用一致 ---
ZiweiLLMRequest = SignableAPIRequest
...
class LLMExtractionResult(BaseModel):
    ...
    is_naming_question: bool = Field(False, description="查询是否是关于给某人或某物取名。")
    is_capability_inquiry: bool = Field(False, description="判断用户是否在询问AI自身的功能、能力或身份。")
    is_knowledge_question: bool = Field(False, description="判断用户是否在询问命理专业术语、概念解释、理论知识。")
...
class MultiTimeAnalysisResult(BaseModel):
    ...
    suggestion_options: Optional[List[str]] = Field(None, description="大模型生成的选项列表")
...
class DetailedIntentExtractionResult(BaseModel):
    ...
    is_next_decadal_query: Optional[bool] = Field(None, description="判断是否询问下一个大运")
    relative_time_indicator: Optional[str] = Field(None, description="提取的原始时间指示器")
    target_year: Optional[int] = Field(None, description="解析后的目标年份")
    target_month: Optional[int] = Field(None, description="解析后的目标月份")
    target_day: Optional[int] = Field(None, description="解析后的目标日期")
    target_hour: Optional[int] = Field(None, description="解析后的目标小时")
    target_minute: Optional[int] = Field(None, description="解析后的目标分钟")
    resolved_horoscope_date: Optional[str] = Field(None, description="解析后的完整日期时间字符串")
    analysis_level: Optional[Literal["yearly", "monthly", "daily"]] = Field(None, description="分析级别")
